...
## World-space reflection density experiment

Adds an opt-in World-space Reflection Cache (Experimental) toggle under cloud Quality & performance, backed by rtx.atmosphere.cloudReflectionDensityCache (default false). It bakes a 128x64x128 RGBA16F density/profile/detail/SDF grid every frame (8 MiB), then samples it during the existing secondary dome traversal. This is a representation prototype, not direct secondary-ray traversal and not a demonstrated speedup. The dome resolution, sample budget, mip chain, existing lighting and second cloud layer remain on their existing paths. No temporal reuse or update cadence reduction is introduced.

Cache sampling is bounded to the world-space grid, with procedural fallback outside it or for low interpolated density. Cached SDF is never used to enlarge empty-space steps. Filtering can soften dense boundaries and fine detail. The bake evaluates local coverage/type fields rather than the reference marcher's interpolation between ray endpoints, so spatial variation can differ. Camera-distance-dependent detail is rebuilt each frame. The cache only targets first-layer view density; moon lighting and other procedural lighting costs remain.

Compare the sum of AtmosphereCloudReflectionDensity and AtmosphereCloudSecondaryLut against the reference AtmosphereCloudSecondaryLut stage. Presence of the density stage identifies experimental sampled frames, even when delayed timestamp logs arrive across a toggle. Periodic mode logs provide supplementary context. A reduction in traversal time alone is not a win. Rejected if total cost or visual quality is worse; disabled mode does not allocate or dispatch the cache (an already allocated cache remains resident after toggling off).

Validation: release Meson build passed; all 10 affected cloud SPIR-V modules passed Vulkan 1.3 scalar-block-layout validation and their exact bytes were found in the final DLL. Runtime performance and appearance remain unverified. In one session, keep Fast Cloud Reflections enabled and use a fixed reflective view: experiment off for 20 seconds, on for 20 seconds, off for 20 seconds. Leave the game running for log collection.

## Shared ambient column integration experiment

Follow-up to the user's full-coverage slowdown: low base cloud samples do not cap the active adaptive marcher (cloudViewSamplesMax does), and the two 256x32x256 lighting bakes have independent density-sample counts. Coverage removes cheap empty-density exits. These are source-confirmed cost mechanisms, not attribution of the reported FPS loss; the user said current timing logs are unreliable.

A general shared density volume is not an exact substitute for both bakes. Sun rays freeze coverage/type controls at the integration origin, cross columns, and travel beyond the 12 km local grid (up to 40 km sampled plus an extrapolated tail). Baking local controls into a wrapped density cache would change the field and can invent repeated shadows. The ambient integral stays within its column and shares its controls, making it the bounded first implementation.

Add rtx.atmosphere.cloudAmbientColumnScan (false by default), exposed as Shared Ambient Cloud Integration (Experimental). A compile-time shader variant evaluates 64 full-fidelity density samples per XZ column and uses a groupshared suffix scan to derive all 32 ambient optical-depth outputs. Partial cells are integrated at each actual curved-grid output height. The existing integration's flat upper plane is preserved to isolate this change. Dispatch is 4 columns x 64 vertical lanes; shared arrays total 2080 bytes per group. The output resource, consumers, and update cadence remain unchanged; toggling forces a fresh bake even with granularity caching enabled.

Nominal procedural density evaluations fall from 256*256*32*6 = 12,582,912 to 256*256*64 = 4,194,304 per ambient bake. Six scan stages introduce group barriers and shared-memory arithmetic, so this is not a measured 3x speedup. The sun bake, reflection dome, and visible cloud march are unchanged. Sampling changes from six midpoints per output interval to a shared 64-cell piecewise-constant density integral: shading may differ, especially near the slab top where the old six taps can be more closely spaced. This is optional pending visual and GPU-cost comparison.
