...
## Selectable prototype implemented (2026-09-13)

Set `rtx.sharc.fuseAssembly = True` before launch. Default is false. The full-feature TraceRay query has four dedicated fused variants covering SER and WBOIT; the original hit/miss shaders, SHARC update/resolve, bounce counts, and cache estimator remain unchanged. Primary assembly uses the shared helper, with per-invocation radiance rounded through float16 before consumption. Raw radiance is still written for secondary demodulation. Both successful queries and selected-surface misses reach primary assembly.

Throughput receives an independent rgba16f allocation when requested at resource creation (8 bytes per render pixel, excluding allocation overhead). This avoids its previous alias with primary diffuse output. If enabled after allocation, fusion falls back until resources are recreated. False always selects standalone assembly; disabling in a session created with fusion retains the independent allocation.

Effective fusion requires active full-feature SHARC, TraceRay queries, statistics off, sparse rendering off, debug view disabled, NRC/ReSTIR GI inactive, and nonaliased throughput/diffuse views. Only effective activation skips standalone assembly. The runtime logs `[SHARC] Combined query/assembly active` on activation. Unsupported combinations retain standalone assembly.

NEE candidate/sample reads are separate from atomic task feedback writes (InterlockedMax and InterlockedAdd); fusion retains those operations and does not consume updated task feedback as current candidates. Invocation ordering changes, so temporal convergence still needs runtime comparison.

Validation: Release Meson build passed; all 70 selected SHARC SPIR-V blobs are embedded in the final DLL and omit 14 foreign backend descriptors. All four fused variants retain raw output 190 and primary outputs 249/250. Existing nonfused SHARC shader blobs match the pre-extraction snapshot exactly. Only the four assembly blobs differ from that snapshot due to the prior shared-helper extraction. Direct3DCreate9 creation/release smoke test passed, and RtxOptions.md was regenerated by the runtime.

Test the same save and view, then move past emissive lights, glass, and reflective surfaces. Inspect dark corners, primary sky misses, and camera motion. Allow 45-60 seconds stationary for stage samples. Compare total GPU time and IndirectIntegration + IndirectAssembly across split/fused: the fused integration interval now includes assembly work. Do not interpret a larger query interval alone as regression. Bit-identical images, half-conversion edge cases, GPU execution correctness, temporal quality, and performance are not yet established.
