...
- **Inline block** at `SceneManager::submitExternalDraw` (between the replacement early-out and the submesh loop) — ~120 LOC (2026-07-02). Wires the preserve fast path (`rtx.enablePreservePath`) into external API draws: mirrors `submitDrawState`'s gate for single-submesh, particle-free draws (batched `instancesToObject` draws are eligible too as of 2026-07-20, gated by `rtx.enablePreservePathForBatchedDraws`, default on — the identity hash folds in the whole GPU-instancing transform array, so a preserved batch is bit-identical to last frame; AccelManager's point-instancer expansion + dynamic-BLAS reuse are path-independent, and preserve skips only the `processDrawCallState` retranslation; same-day fix: `computeExternalDrawIdentityHash` gained an `instancesToObject` fallback because `submitExternalDraw` moves `gpuInstancingTransforms` into the drawCall BEFORE hashing — without it every batched draw hashed its transform array as absent, and under preserve a batch whose membership changed rendered a stale instance array, i.e. vanishing/ghost statics) whose `ReplacementInstance` matched by exact identity this frame (`dirtyFlags.isClear()`), whose single `RtInstance`/BLAS survives from a prior dynamic frame, whose BLAS was not already touched by a sibling draw this frame, whose effective material is not converted to a light, and while the texture-cache generation matches. On the gate passing, refreshes `BlasEntry::input` from this frame's submesh and dispatches `preserveReplacementInstance`, skipping the full `processDrawCallState` retranslation. Includes a periodic (~15s) `[RTX-Preserve] external draws` preserved-vs-dynamic telemetry log.
  *Exceeds the 20-LOC inline guidance — kept inline (for now) because the gate needs `preserveReplacementInstance` plus the private `m_drawCallTracker` / `m_pReplacer` / `m_terrainBaker` / `m_textureCacheGenerationValidForPreserve` members; lifting it into `rtx_fork_submit.cpp` would require a wide `friend` grant. Flag for hook-refactor or an upstream feature PR. Known v1 gap: `fork_hooks::externalDrawTextureCategories` runs inside the dynamic loop, so mid-session texture-category edits do not re-categorize draws that keep taking the preserve path.*
